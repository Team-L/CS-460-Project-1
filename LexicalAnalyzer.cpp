...
int LexicalAnalyzer::detectEndOfLexeme(int state_found, int i, string lexeme, char a, int previous_state)
{
  ...
  switch(state_found)
    ...
    case end_IDKEY_T:
      ...
      p1 << "\t" << left << setw(16)  << GetTokenName(token) << lexeme << endl;
    ...
    case IDKEY_T:
      ...
      // only the vector can preserve the "?" at the end of the lexeme
      lexemes.push_back(lexeme);
      token = IDENT;
    ...
    case error:
      ...
      // these are states that require peeking to detect end
